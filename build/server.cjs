...
app.get("/sync", async (req, res) => {
    ...
    for (let key in data) {
        ...
        for (let i = 1; i < data[key].length; i++) {
            let mone = await (0, database_1.getMoneByIdOrNumber)(data[key][i].id);
            if (mone) {
                resultObj[key].push(mone);
            }
            else {
                await (0, database_1.addMone)(data[key][i]);
...
app.get("/getMone/:id", async (req, res) => {
    ...
    let mone = await (0, database_1.getMoneByIdOrNumber)(id);
    res.json(mone);
});
app.post("/updatMone", async (req, res) => {
    let mone = req.body;
    await (0, database_1.updateMone)(mone.id, mone);
    res.status(200).json({ status: "good" });
});
app.get("/all", async (req, res) => {
    let all = await (0, database_1.getAllMonim)();
    res.json({ all });
});
app.post("/sendWhathapp", async (req, res) => {
    let _phone = req.body.phone;
    let message = req.body.message;
    let mone = await (0, database_1.getMoneByIdOrNumber)(req.body.moneId);
    if (!_phone) {
        return res.status(400).json({ x: 'Phone is required' });
    }
