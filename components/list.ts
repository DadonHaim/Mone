function lensNew(arr:IMone[]) {
    let res = 0;
    for(let i=1;i<arr.length;i++){if(arr[i].read==false && arr[i].unRead=="--") res++}
    return res
}
...
function list(){
    ...
    for(let key in AllMonesObj){
        ...
        let mones = AllMonesObj[key]  as IMone[]
        ...
        var item =  createElement({
            ...
            dad: List,
            children : mones.mapFrom(1,(mone:IMone)=>{
...
function wazeCategory(arr:IMone[]){
    let addresses = []
    arr.forEach(mone=>{
        if(!mone) return
        if(!mone.city) return
